...
import { Todo, TodoResponse } from "@/src/shared/types/todo"
...
export const DashBoardAllQuizes: FC<ComponentProps> = ({ items }) => {
  const [animationParent] = useAutoAnimate()
  const router = useRouter()
  const headerTable = ["id", "userId", "Заголовок", "Выполнено", "Действия"]
  const [selectedItems, selectedItemsSet] = useState<Todo[]>([])

  const onSelect = (item: Todo) => (e: ChangeEvent<HTMLInputElement>) => {
    ...
    <>
                ...
                <th
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      checked={selectedItems.length === items.todos.length}
              ...
              {items.todos.map((item, indx) => {
                return (
                  <tr
                    ...
                    <th
                      scope="row"
                      className="flex items-center justify-start whitespace-nowrap px-6 py-4 text-gray-900 dark:text-white">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center">{item.todo}</div>
                    </td>
      ...
      <nav
        ...
        <span className="text-sm font-normal text-gray-500 dark:text-gray-400">
          Показано <span className="font-semibold text-gray-900 dark:text-white">20</span> из <span className="font-semibold text-gray-900 dark:text-white">{items.total}</span>
        </span>

        <ul className="inline-flex items-stretch -space-x-px">
          {Array.from({ length: items.total / items.limit }).map((_item, indx) => {
            return (
              <li key={indx}>
                <Link
                  href={`${router.pathname}?page=${indx + 1}`}
