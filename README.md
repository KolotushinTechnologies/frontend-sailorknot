...
Не отправлять поле statusChangeFile
пустой массив documents
пустой массив fileNames
Условие.
Если у всех документов поле isNew === false 
&&
Если длинна выбранных документов === длинна документов профиля
...
пустой массив fileNames
Условие.
Если длинна выбранных документов = 0, но у профиля > 0  

Изменить документы / добавить новые
statusChangeFile=true 
заполненный массив documents
заполненный массив fileNames
Условие.
Если у одного из документов поле isNew === true
ИЛИ
Если длинна выбранных документов > 0 && у профиля 0 документов
...
const Page: NextPageWithLayout<PageProps> = () => {
  ...
  const onSubmit = handleSubmit(async (data, event) => {
    ...
    // Изменить документы / Добавить новые
    if ((selectedImages.length > 0 && profileData && profileData.documents.length === 0) || selectedImages.some((search) => search.isNew === true)) {
      const toastId = toast.loading("Обновление...", {
        position: "top-right",
      })
      try {
